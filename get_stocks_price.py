...
# STOCKS_PRICE_HDF5 = '/home/cloga0216/blog/blog/fintech/fin_tech.h5'
sh_index_data = ts.get_k_data('000001', index=True)
sh_index_data_m = ts.get_k_data('000001', index=True, ktype='M')
sh_index_data_m = sh_index_data_m.set_index('date')
sh_index_data_m['pre_close'] = sh_index_data_m['close'].shift(1)
sh_index_data_m['return%'] = (sh_index_data_m['close'] - sh_index_data_m['pre_close']) / sh_index_data_m['pre_close'] * 100
sh_index_data = sh_index_data.set_index('date')
sh_index_data.reset_index().to_hdf(STOCKS_PRICE_HDF5, 'sh_index_data', format='table', data_columns=True)
sh_index_data_m.reset_index().to_hdf(STOCKS_PRICE_HDF5_MONTH, 'sh_index_data', format='table', data_columns=True)
# 算一下beta数据
stock_beta = []
for i, code in enumerate(stock_basics['code']):
    ...
    if len(k_data) == 0 or len(k_data_m) == 0:
        corr_coef = 0
        beta_overall = 0
        beta = 0
        continue
